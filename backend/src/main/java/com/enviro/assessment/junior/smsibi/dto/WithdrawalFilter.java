 ...
 * {@code /api/withdrawals?investorId=1&from=2026-01-01&to=2026-03-31}.
 */
public record WithdrawalFilter(
        @Parameter(description = "Only withdrawals for this investor")
        Long investorId,

        @Parameter(description = "Only withdrawals from this product")
        Long productId,

        @Parameter(description = "Start date, inclusive (yyyy-MM-dd)") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
        LocalDate from,

        @Parameter(description = "End date, inclusive (yyyy-MM-dd)") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
        LocalDate to) {

    /** No filters at all: returns every withdrawal. */
    public static WithdrawalFilter none() {
        return new WithdrawalFilter(null, null, null, null);
    }
